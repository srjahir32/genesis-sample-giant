...
GitHub project link: https://github.com/studiopress/genesis-giant/.


## Installation Instructions

1. Upload the Genesis Giant theme folder via FTP to your wp-content/themes/ directory. (The Genesis parent theme needs to be in the wp-content/themes/ directory as well.)
2. Go to your WordPress dashboard and select Appearance.
3. Activate the Genesis Giant theme.
...
The version of [Genesis Giant on GitHub](https://github.com/studiopress/genesis-giant/) includes tooling to check code against WordPress standards. To use it:
...
2. In the command line, change directory to the Genesis Giant folder.
...
You'll see output highlighting issues with PHP files that do not conform to Genesis Giant coding standards.
...
- `npm run makepot` to regenerate the `languages/genesis-giant.pot` file.
...
- `npm run zip` to create a genesis-giant.zip. Files in the `excludes` array in `scripts/makezip.js` are omitted.
...
4. Type `npm run zip` to create `genesis-giant.zip`. Files in the `excludes` array in `scripts/makezip.js` are omitted from the zip. `filename.md` files will be renamed to `filename.txt`.
